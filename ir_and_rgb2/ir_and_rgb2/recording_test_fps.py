...
def main(**kwargs):
    ...
    # Keep reading for 120 seconds
    while True:
        image, temp, dt = worker.read()
        cur_time = time.time()

        if start_time is not None and cur_time - start_time > 120:
            break
        ...
        if worker.idx == 1: # if it is the first time being read
            ...
            if abs(interval) >= 1: # wait for a second so that worker.idx - last_idx is not just 1
                ...
                timestamp_list.append(cur_time)
                fps_list.append(fps)
                lts_rdr = timestamp
                last_idx = worker.idx
    ...
    # plot fps against time, and plot a histogram of fps; draw them on the same figure, and save to /output folder
    fig, axes = plt.subplots(nrows=1, ncols=2)
    axes[0].plot(timestamp_list, fps_list)
    axes[0].set_xlabel("time")
    axes[0].set_ylabel("fps")
    axes[0].set_title("fps against time")

    axes[1].hist(fps_list)
    axes[1].set_xlabel("fps")
    axes[1].set_ylabel("count")
    axes[1].set_title("histogram of fps")

    # compute the average fps
    avg_fps = sum(fps_list) / len(fps_list)

    # add average fps as text on the bottom of the first plot
    axes[0].text(0.5, 0.05, "average fps: {:.2f}".format(avg_fps), transform=axes[0].transAxes)

    fig.tight_layout()
    fig.savefig("output/recording/b.png")
    plt.close(fig)
